...
int board_late_init(void)
{
	...
	setenv("board_name", "MX6UL_CHARGE");
		...
		switch (var_eeprom_config_struct_v2.som_info &0x3){
			...
			case 0x02:
				snprintf(fdt_filename, FDT_FILENAME_MAX_LEN, "%s",
					...
					"imx6ull-charge-sd_emmc.dtb" :
					"imx6ul-charge-sd_emmc.dtb");
			break;
			case 0x01:
				snprintf(fdt_filename, FDT_FILENAME_MAX_LEN, "%s",
					imxtype == MXC_CPU_MX6ULL ?
					"imx6ull-charge-sd_nand.dtb" :
					"imx6ul-charge-sd_nand.dtb");
			break;
		}
		break;
	case VBOOT_DEVICE_MMC:
		...
		if (var_eeprom_config_struct_v2.som_info & 0x4)
				snprintf(fdt_filename, FDT_FILENAME_MAX_LEN, "%s",
					imxtype == MXC_CPU_MX6ULL ?
					"imx6ull-charge-emmc_wifi.dtb" :
					"imx6ul-charge-emmc_wifi.dtb");
		else
				snprintf(fdt_filename, FDT_FILENAME_MAX_LEN, "%s",
					imxtype == MXC_CPU_MX6ULL ?
					"imx6ull-charge-sd_emmc.dtb" :
	...
	case VBOOT_DEVICE_NAND:
		...
		if (var_eeprom_config_struct_v2.som_info & 0x4)
				snprintf(fdt_filename,FDT_FILENAME_MAX_LEN,"%s",
					imxtype == MXC_CPU_MX6ULL ?
					"imx6ull-charge-nand_wifi.dtb" :
					"imx6ul-charge-nand_wifi.dtb");
		else
				snprintf(fdt_filename,FDT_FILENAME_MAX_LEN,"%s",
					imxtype == MXC_CPU_MX6ULL ?
					"imx6ull-charge-sd_nand.dtb" :
...
static int  spl_dram_init_v2(void)
{
	...
	/* Add here: Read EEPROM and parse Charge struct */
	...
	if(var_eeprom_config_struct_v2.charge_magic!=0x32524156) //Test for VAR2 in the header.

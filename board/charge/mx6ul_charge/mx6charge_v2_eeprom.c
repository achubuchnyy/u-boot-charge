...
int var_eeprom_v2_read_struct(struct var_eeprom_config_struct_v2_type *var_eeprom_config_struct_v2)
{
	...
	eeprom_found = i2c_probe(CHARGE_MX6_EEPROM_CHIP);
	if (0 == eeprom_found)
	{
		if (i2c_read(CHARGE_MX6_EEPROM_CHIP, 0, 1, (void*)var_eeprom_config_struct_v2,sizeof(struct var_eeprom_config_struct_v2_type)))
		{
...
int var_eeprom_write(uchar *ptr, u32 size, u32 offset)
{
	...
	while ((0 == ret) && (size_written < size_to_write))
	{
		...
		chip = CHARGE_MX6_EEPROM_CHIP + P0_select_page_EEPROM;
		addr = (offset & 0xFF);
		ret = i2c_write(chip, addr, 1, ptr, CHARGE_MX6_EEPROM_WRITE_MAX_SIZE);

		/* Wait for EEPROM write operation to complete (No ACK) */
		udelay(11000);

		size_written += CHARGE_MX6_EEPROM_WRITE_MAX_SIZE;
		offset += CHARGE_MX6_EEPROM_WRITE_MAX_SIZE;
		ptr += CHARGE_MX6_EEPROM_WRITE_MAX_SIZE;
	}
...
static int do_var_eeprom_params(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (var_eeprom_write((uchar *)&var_eeprom_cfg.part_number[0],
				...
				CHARGE_MX6_EEPROM_STRUCT_OFFSET + offset) )
	{
